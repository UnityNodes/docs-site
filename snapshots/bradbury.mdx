...
<Accordion title="How do I check the node's sync status?">
  ...
  Look for a line that reads `INF Node is synced!!!` - that means you have caught up.

  You can also grab just the most recent processed block or the last "synced" event:

  ```bash
  journalctl -u genlayer-node -n 500 --no-pager \
    | grep -oE 'blockNumber=[0-9]+' | tail -1

  journalctl -u genlayer-node --no-pager \
    | grep "Node is synced" | tail -1
  ```

  Compare the block number with the **Network head** shown in the widget at the top of this page - if they are close, you are in sync.
</Accordion>
