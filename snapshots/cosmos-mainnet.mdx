...
| What | Setting |
|---|---|
| **Binary** | `gaiad` at the version the snapshot was taken on. Every archive has a `.txt` sidecar with `node_version` (currently `v28.1.0`); a node started on an older binary than the state was written with will not start |
